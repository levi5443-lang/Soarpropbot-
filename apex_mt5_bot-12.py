...
APEX_BOT_URL      = os.environ.get("APEX_BOT_URL", "")   # e.g. https://apex-bot.onrender.com
...
# Grade-based execution delay (seconds max) — mirrors commercial bot
# Prevents detectable entry clustering; higher grade = faster execution
def grade_delay_max(grade: float) -> int:
    if grade >= 9.0:  return 5
    if grade >= 8.0:  return 15
    if grade >= 7.0:  return 30
    if grade >= 6.5:  return 60
    return 0

# Stop loss randomisation — ±pip offset to avoid predictable SL levels
SL_RANDOMISE_PIPS = {
    "XAUUSD":  5,    # ±5 pips on Gold
    "default": 3,    # ±3 pips on forex
}

def randomise_stop_loss(pair: str, stop_loss: float, direction: str) -> float:
    """Apply a small random pip offset to the stop loss."""
    max_pips    = SL_RANDOMISE_PIPS.get(pair, SL_RANDOMISE_PIPS["default"])
    pip_size    = 0.1 if "XAU" in pair else 0.0001
    offset_pips = random.uniform(-max_pips, max_pips)
    offset      = offset_pips * pip_size
    if direction == "BUY":
        # BUY: SL is below entry — move it slightly further down (worse) or up (tighter)
        new_sl = stop_loss + offset
    else:
        # SELL: SL is above entry
        new_sl = stop_loss - offset
    return round(new_sl, 2 if "XAU" in pair else 5)
...
async def notify_apex(signal_id: str, mt5_order_id: str, status: str,
                      pnl_pips: float = None, exit_price: float = None):
    """
    POST outcome back to the apex bot so Levz knows the order was placed
    and can track/manage it. Prevents false EXPIRED marks on filled trades.
    """
    if not APEX_BOT_URL:
        return
    try:
        ...
        url = APEX_BOT_URL.rstrip("/") + "/executor/callback"
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.post(url, json=payload,
                                  headers={"X-Secret": WEBHOOK_SECRET})
            log.info("Apex callback: " + status + " → " + str(r.status_code))
    except Exception as e:
        log.warning("Apex callback failed (non-critical): " + str(e))
...
async def process_signal(bot, signal: dict):
    ...
    # Grade-based execution delay — mirrors commercial bot timing
    try:
        grade_float = float(signal.get("score", signal.get("grade", 0)))
    except (TypeError, ValueError):
        grade_float = 0.0
    delay_max = grade_delay_max(grade_float)
    if delay_max > 0:
        delay = random.uniform(0, delay_max)
        log.info("Grade delay: " + str(round(delay, 1)) + "s (max " + str(delay_max) + "s for grade " + str(grade_float) + ")")
        await asyncio.sleep(delay)

    # SL randomisation — avoid predictable stop levels
    sl_original = signal.get("stop_loss")
    if sl_original:
        sl_randomised = randomise_stop_loss(pair, float(sl_original), dirn)
        signal = dict(signal)
        signal["stop_loss"] = sl_randomised
        log.info("SL randomised: " + str(sl_original) + " → " + str(sl_randomised))
        # Recalculate lot size with randomised SL so risk stays accurate
        lot_size = calc_lot_size(pair, entry, sl_randomised, balance, risk_pct)
...
async def execute_trade(bot, signal: dict, lot_size: float, risk_usd: float):
    ...
    try:
        ...
        # Notify apex bot so Levz knows this signal has a live MT5 order
        asyncio.create_task(notify_apex(
    ...
    for trade in open_trades:
        ...
        if pos_id and pos_id not in open_position_ids and pos_id != "":
            ...
            try:
                ...
                # Notify apex bot so Levz records correct WIN/LOSS outcome
                asyncio.create_task(notify_apex(
                    signal_id    = trade.get("signal_id", ""),
                    mt5_order_id = str(trade.get("order_id", "")),
                    status       = result,
                    pnl_pips     = round(pnl_pips, 1),
                    exit_price   = round(current, 5),
                ))
        ...
        if age_hours >= valid_h:
            if order_id in live_order_ids:
                ...
                try:
                    ...
                    # Tell apex bot this signal expired unfilled
                    asyncio.create_task(notify_apex(
                        signal_id    = trade.get("signal_id", ""),
                        mt5_order_id = order_id,
                        status       = "EXPIRED",
