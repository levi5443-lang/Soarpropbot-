...
# Soar Funding 2-Step rules — dynamic based on live balance
ACCOUNT_BALANCE_START  = 10000.0    # starting balance for reference
...
# Static fallback values (used before first balance fetch)
MAX_RISK_USD           = ACCOUNT_BALANCE_START * MAX_RISK_PCT           # $300
MAX_DAILY_LOSS_USD     = ACCOUNT_BALANCE_START * MAX_DAILY_LOSS_PCT     # $400
MAX_TOTAL_LOSS_USD     = ACCOUNT_BALANCE_START * MAX_TOTAL_LOSS_PCT     # $800
ALERT_DAILY_USD        = ACCOUNT_BALANCE_START * (MAX_DAILY_LOSS_PCT - ALERT_BUFFER_PCT)
ALERT_TOTAL_USD        = ACCOUNT_BALANCE_START * (MAX_TOTAL_LOSS_PCT - ALERT_BUFFER_PCT)

def dynamic_limits(balance: float) -> dict:
    """Calculate all limits dynamically from current balance."""
    return {
...
# Daily closed P&L tracker — resets at midnight UTC (6 PM MDT)
DAILY_CLOSED_PNL: float = 0.0   # cumulative closed P&L for today
DAILY_RESET_DATE: str   = ""    # date of last reset (UTC date string)


def check_daily_reset():
    """Reset daily closed P&L tracker at midnight UTC."""
    global DAILY_CLOSED_PNL, DAILY_RESET_DATE
    today = datetime.utcnow().strftime("%Y-%m-%d")
    if DAILY_RESET_DATE != today:
        DAILY_CLOSED_PNL = 0.0
        DAILY_RESET_DATE = today
        log.info("Daily P&L tracker reset for " + today)


def record_closed_pnl(pnl_usd: float):
    """Record a closed trade's P&L to today's running total."""
    global DAILY_CLOSED_PNL
    check_daily_reset()
    DAILY_CLOSED_PNL += pnl_usd
    log.info("Daily closed P&L updated: $" + str(round(DAILY_CLOSED_PNL, 2)))


def get_daily_closed_loss() -> float:
    """Return today's total closed losses as a positive number."""
    check_daily_reset()
    return abs(min(DAILY_CLOSED_PNL, 0))
...
async def monitor_order_expiry(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    pending_trades = [
        ...
        if t.get("status") == "OPEN" and t.get("filled") == False and t.get("order_id", "unknown") != "unknown"
    ]
    ...
    for trade in pending_trades:
        ...
        # Check if order is still pending and has expired
        if age_hours >= valid_h:
            # Try to cancel via MetaAPI
            if order_id in live_order_ids:
                try:
                    ...
                    await ctx.bot.send_message(
                        ...
                        text=(
                            ...
                            "Price never reached the entry level. Setup invalidated."
                        ),
                        parse_mode=ParseMode.MARKDOWN,
                    )
                except Exception as e:
                ...
                # Order not in live list — it either filled or was already cancelled
                # Mark as filled so we stop tracking it
                trade["filled"] = True
                log.info("Order " + order_id + " no longer pending — marking as filled")
